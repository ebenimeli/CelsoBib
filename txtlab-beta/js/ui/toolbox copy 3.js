...
async function tryLoadMainForId(id) {
  ...
  // Mostramos estado accesible mientras se intenta cargar
  ...
  try {
    ...
    // Re-ejecutar <script> (inline o con src) para que corran
    ...
    // Notificar a otros módulos que #main ha cambiado (p. ej., write.js)
 ...
 * excepto cuando el id es "write-mode" (lo gestiona su propio loader).
 */
export function mountToolbox(selectorOrTpl) {
  ...
  // Excepción: no cargar main para write-mode (tiene su propia gestión)
  //if (tpl.id !== "write-mode") {
    // p. ej., template id="game-mode" → assets/main/game-mode.html
  //  tryLoadMainForId(tpl.id); // fire-and-forget
  //}

  return true;
}

/** Click delegación: cuando se pulsa un botón .tool, monta su template */
export function initToolboxDelegation() {
  document.addEventListener("click", (e) => {
    ...
    // Excepción: write-mode se gestiona fuera
    //if (id === "write-mode") return;
